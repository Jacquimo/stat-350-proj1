...
for (n in sample_n) {  
  ...
  hist(mean_dist, xlab="Sample xbar", ylab="Frequency", 
       main=paste("Sample Mean Exponential Distribution for N=", n), las=1, xlim=c(0, 3))
  qqnorm(mean_dist, main=paste("Sample Mean Exponential Quantile Plot for N=", n), las=1)
  qqline(mean_dist)
}
