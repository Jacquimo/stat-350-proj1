...
for (n in sample_n) {  
  ...
  for (i in 1:1000) {
    dist <- runif(n, max=1, min=0) # make the uniform distribution values
    mean <- mean(dist)
  ...
  hist(mean_dist, xlab="Sample xbar", ylab="Frequency", 
       main=paste("Sample Mean Uniform Distribution for N=", n), las=1, xlim=c(0, 1))
  qqnorm(mean_dist, main=paste("Sample Mean Uniform Quantile Plot for N=", n), las=1)
  qqline(mean_dist)
}
